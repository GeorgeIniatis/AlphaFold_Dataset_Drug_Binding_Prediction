...
with useful_info:
    ...
    st.markdown("""
                - Classification models make use of **"logKd"** as the label
                - All models were optimised using [BayesSearchCV](https://scikit-optimize.github.io/stable/modules/generated/skopt.BayesSearchCV.html) for **R2 score** using a **5-Fold Cross-Validation** except in the case of the Dummy Regressor and Linear Regression models
        ...
        if baseline_chosen_model != "-":
            ...
            if st.button("Predict", key="baseline_button"):
                with baseline_prediction_metrics:
                    model = load_model(baseline_chosen_model, "Regression", "Baseline_Models")
                    drug_descriptors = get_chemical_descriptors(drug_cid, "Regression")
                    ...
                    if isinstance(drug_descriptors, str):
                        ...
                        protein_descriptors = get_protein_descriptors(protein_accession, "Regression",
                                                                      "Baseline_Models")

                        descriptors = pd.concat(objs=[drug_descriptors, protein_descriptors])
                        regression_result_column_section(model, baseline_chosen_model, descriptors, "Baseline_Models")
...
with enhanced_models:
    ...
    st.markdown("##### Training Performance")
    baseline_training_performance = pd.read_csv(
        "Streamlit_App/data/Metrics/Regression_Enhanced_Models_Training_Metrics.csv", skiprows=1)
    st.write(baseline_training_performance)

    st.markdown("##### Testing Performance")
    baseline_testing_performance_ = pd.read_csv(
        "Streamlit_App/data/Metrics/Regression_Enhanced_Models_Testing_Metrics.csv", skiprows=1)
    st.write(baseline_testing_performance_)
        ...
        if enhanced_chosen_model != "-":
            ...
            if st.button("Predict", key="enhanced_button"):
                with enhanced_prediction_metrics:
                    model = load_model(enhanced_chosen_model, "Regression", "Enhanced_Models")
                    drug_descriptors = get_chemical_descriptors(drug_cid, "Regression")
                    ...
                    if isinstance(drug_descriptors, str):
                        ...
                        descriptors = pd.concat(objs=[drug_descriptors, protein_descriptors])
                        regression_result_column_section(model, enhanced_chosen_model, descriptors, "Enhanced_Models")

with training_process:
    st.subheader("Training & Testing Process Overview")
    st.markdown("""
                * Confusion Matrix & Error Analysis do not apply for the Regression models
                """)
